...
| 2026-08-26 | 文件存储详情新增“权限”Tab，按 `产品原型-8.25` 展示访问规则和默认目录权限；ANI 尚未开放 NFS export / POSIX ACL 接口，因此保持契约空态并禁用新增、编辑操作。 |
| 2026-08-25 | 产品原型基线更新为 GitNexus 索引 `产品原型-8.25`；同步修复侧边菜单图标重复，并统一向量存储菜单、列表与详情使用有效的 `xiangliangcunchu` 图标。 |
| 2026-08-25 | 块存储列表与详情补齐真实扩容、创建快照、OS 初始化引导/完成及挂载/卸载流程：复用共享弹窗并携带独立幂等键，关联实例改为带行操作的表格；基本信息按 ANI 响应展示容量、类型、可用区、IOPS、加密、自动快照、来源快照、挂载实例、OS 初始化和约束，并同步 Core 类型快照。 |
| 2026-08-25 | 文件存储按 `产品原型-8.24` 完成列表、详情和创建流程收敛：列表展示性能模式与真实挂载目标数，列表/详情均支持扩容和创建挂载目标；创建文件存储时联动选择真实 VPC/子网并继续创建挂载目标，所有副作用请求使用独立幂等键。详情 Tab 收敛为“挂载目标 / 权限 / 事件”，权限因 ANI 暂无接口保留待定空态，未添加虚假删除或配置操作。 |
| 2026-08-25 | 向量存储列表、创建与详情对齐真实契约和原型：同步 Embedding 模型、向量数、索引状态、最近索引时间与关联知识库字段；创建时从真实模型仓库选择 Embedding 模型并提交，详情以“索引 / 检索测试 / 关联”等独立区域展示真实数据与搜索能力。 |
| 2026-08-25 | 统一块存储、文件存储、对象、向量存储、推理服务和实例操作详情的状态原因展示，将原因收敛到状态标签悬停提示。今日变更已通过 TypeScript `tsc -p tsconfig.app.json --noEmit`、`git diff --check` 与 GitNexus 变更检测；未运行 production build 或启动开发服务。 |
| 2026-08-24 | 新增 `scripts/build-and-push-image.sh`，用于在 WSL/Linux 中以 `YYYYMMDDHHMM` 时间标签构建并推送 `docker.changqingyun.cn/ani/ani-console`，并固定 Shell 脚本使用 LF 换行；支持自动选择 Docker/Buildah、按可用权限决定是否使用 sudo，以及自定义工具、平台、仓库和标签。Bash 语法、帮助及非法标签检查、TypeScript typecheck、`git diff --check` 与 GitNexus 变更检测通过，未实际执行镜像构建或推送。 |
| 2026-08-24 | 将 Console 产品基线更新至 `产品原型-8.24` 并完成当日功能收敛：AI 服务接入真实模型与推理服务契约，提供模型列表、推理服务查询与生命周期管理、共享单页部署表单及租户 Registry 镜像匹配，隐藏后端未开放的策略、模型上传和调用地址能力；新增知识库列表、创建、详情、文档直传与解析轮询、删除及 SSE 问答；镜像仓库改为真实 Tag 平铺管理并支持筛选、推送说明、拉取命令、用途跳转和引用冲突保护删除；移除演示、预留、监控与告警、安全与身份、用量等不再保留的菜单、页面及首页面板；补齐文件存储性能模式与协议，统一文件/向量存储状态筛选，并保留仅开发环境可用的跳过登录入口。 |
