...
export function createMockVmInstanceDataSource(
  seed: VmInstance[] = vmInstanceSeed,
): VmInstanceDataSource {
  ...
  return {
    async list(query: VmInstanceQuery): Promise<VmInstanceListResult> {
      ...
      let filtered = instances.filter(
        (item) => query.status === "all" || item.status === query.status,
      );

      if (keyword) {
        filtered = filtered.filter((item) =>
          item[query.searchField].toLocaleLowerCase().includes(keyword),
        );
      }
    ...
    async changePowerState(ids: string[], action: VmInstancePowerAction) {
      ...
      instances = instances.map((item) => {
        if (!idSet.has(item.id)) return item;
        const canChange =
          action === "start" ? item.status !== "running" : item.status === "running";
        return canChange ? { ...item, status: nextStatus } : item;
      });
    },
  };
...
export const vmInstanceDataSource: VmInstanceDataSource = {
  async list(query: VmInstanceQuery): Promise<VmInstanceListResult> {
    await waitForMockResponse();
    const keyword = query.keyword.trim().toLocaleLowerCase();
    let filtered = listSharedVmInstances().filter(
      (item) => query.status === "all" || item.status === query.status,
    );
