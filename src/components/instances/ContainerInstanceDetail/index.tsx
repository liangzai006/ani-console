import { Button, Descriptions, Dropdown, Menu, Message, Modal, Space, Tooltip } from '@arco-design/web-react'
...
export function ContainerInstanceDetailPage({ instanceId }: { instanceId: string }) {
  ...
  const busy = ['pending', 'provisioning', 'starting', 'stopping', 'deleting'].includes(detail.state ?? '')

  const image = detail.image?.ref ?? detail.image?.name ?? detail.image?.id ?? '-'
  const nameValue = detail.compute?.cpu || detail.compute?.memory
        ...
        <Dropdown
          ...
          droplist={(
            <Menu onClickMenuItem={(action) => {
              if (action === 'start' || action === 'stop' || action === 'restart') lifecycle.mutate(action)
              if (action === 'terminal') openTerminalWindow(instanceId)
              if (action === 'delete') {
              Modal.confirm({
                title: '删除实例',
              ...
              <Menu.Item key="start" disabled={detail.state !== 'stopped' && detail.state !== 'failed'}>启动</Menu.Item>
              <Menu.Item key="stop" disabled={!isRunning || detail.termination_protection}>停止</Menu.Item>
              <Menu.Item key="restart" disabled={!isRunning}>重启</Menu.Item>
              <Menu.Item key="terminal" disabled={!isRunning}>打开终端</Menu.Item>
              <Menu.Item key="delete" disabled={busy || detail.termination_protection} style={{ color: 'var(--color-danger-6)' }}>删除</Menu.Item>
            </Menu>
          )}
        >
          <Button loading={lifecycle.isPending || deleteInstance.isPending}>更多操作</Button>
        </Dropdown>
      }
      cards={[
        {
      ...
      tabs={[
        {
          ...
          content: <InstanceReleases instance={detail as components['schemas']['InstanceRecord']} />,
        },
        {
          key: 'network',
          label: '网络',
        ...
        },
        {
          key: 'terminal',
          label: '终端',
          content: <InstanceTerminal instanceId={instanceId} />,
        },
        {
          key: 'operations',
          label: '操作历史',
