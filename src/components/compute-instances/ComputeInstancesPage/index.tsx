...
function createRouteForKind(kindFilter?: InstanceKind): string | null {
  if (kindFilter === "container") return "/container-instances/create";
  if (kindFilter === "vm") return "/vm-instances/create";
  if (kindFilter === "sandbox") return "/sandbox-instances/create";
  return null;
}
...
export function InstanceCreateForm({
  ...
  const subnets = useQuery({
    queryKey: ["network-subnets", "select", form.vpc_id],
    queryFn: () =>
      listOrThrow(() =>
        coreApi.GET("/networks/subnets", {
          params: { query: { limit: 50, vpc_id: form.vpc_id || undefined } },
        }),
      ),
    enabled: form.network_mode === "vpc" && !!form.vpc_id,
  });
