...
export function ContainerInstanceCreatePage() {
  ...
  const subnets = useQuery({
    queryKey: ['network-subnets', 'select', selectedVpcId],
    queryFn: () => listOrThrow(() => coreApi.GET('/networks/subnets', {
      params: { query: { limit: 50, vpc_id: selectedVpcId } },
    })),
    enabled: !!selectedVpcId,
  })
  ...
  const handleValuesChange = (
    ...
    if (Object.prototype.hasOwnProperty.call(changedValues, 'vpc_id')) {
      setSelectedVpcId(values.vpc_id ?? '')
  ...
  return (
    <FormPageFrame<ContainerInstanceFormValues>
      breadcrumbs={[{ label: '容器实例', to: '/container-instances' }, { label: '创建容器实例' }]}
      form={form}
      sections={sections}
      actions={[
