...
export function SubnetDetailPage({ subnetId }: { subnetId: string }) {
  ...
  const instances = useQuery({
    ...
    queryFn: () => listOrThrow(() => coreApi.GET('/instances', {
      params: { query: asUncontractedQuery({ limit: 100, subnet_id: subnetId }) },
    })),
  })
  ...
  const openRelated = (item: (typeof summaryItems)[number]) => {
    ...
    else navigate({ to: '/compute-instances/$instanceId', params: { instanceId: item.id } })
  }
            ...
            <Space direction="vertical" size={12} className="w-full">
              ...
              <DataTable<SubnetRouteRow>
                columns={[
                  ...
                  { title: '操作', render: (_, item) => item.protected ? <Typography.Text type='secondary'>受保护</Typography.Text> : <Button type='text' size='mini' onClick={() => navigate({ to: '/routes' })}>打开</Button> },
                ]}
                data={routeRows}
