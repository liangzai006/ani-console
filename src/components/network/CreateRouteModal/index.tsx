    ...
    >
      <Form layout="vertical">
        <Form.Item label="名称" required>
          <Input value={name} onChange={setName} placeholder="请输入路由名称" maxLength={128} showWordLimit />
        </Form.Item>
        ...
        <Form.Item label="目标网段" required validateStatus={cidrError ? 'error' : undefined} help={cidrError}>
        ...
        <Form.Item label="下一跳 ID" required>
          <Input value={nextHopId} onChange={setNextHopId} placeholder="请输入网关、实例或 NAT 资源 ID" />
        </Form.Item>
        <Typography.Text type="secondary">创建后不可编辑；如需变更目标或下一跳，请删除后重新创建。</Typography.Text>
      </Form>
    </Modal>
  )
}
