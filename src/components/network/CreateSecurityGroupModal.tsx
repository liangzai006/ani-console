...
const RULE_TEMPLATES: Record<RuleTemplate, SecurityGroupRule[]> = {
  common: [
    ...
    { direction: 'ingress', protocol: 'tcp', port_range: '3389', cidr: '0.0.0.0/0', action: 'allow' },
    { direction: 'ingress', protocol: 'icmp', port_range: 'all', cidr: '0.0.0.0/0', action: 'allow' },
  ],
  open: [
    { direction: 'ingress', protocol: 'all', port_range: 'all', cidr: '0.0.0.0/0', action: 'allow' },
    ...
    >
      <Form layout="vertical">
        <Form.Item label="名称" required>
          <Input value={name} onChange={setName} placeholder="请输入安全组名称" maxLength={64} showWordLimit />
        </Form.Item>
        ...
        {vpcs.error ? (
          <Alert type="error" showIcon content={getErrorMessage(vpcs.error, 'VPC 列表加载失败')} className="mb-4" />
        ) : null}
        ...
        <Typography.Text type="secondary">创建后可在安全组详情页逐条添加或编辑入站、出站规则。</Typography.Text>
      </Form>
    </Modal>
  )
}
