...
export function LoadBalancerDetailPage({ loadBalancerId }: { loadBalancerId: string }) {
  ...
  if (!detail.data)
    return (
      <DetailPagePlaceholder
        breadcrumbs={[{ label: '网络' }, { label: '负载均衡', to: '/load-balancers' }, { label: loadBalancerId }]}
  ...
  const openRelated = (related: (typeof summaryItems)[number]) => {
    if (related.type === 'vpc') {
      navigate({ to: '/vpcs/$vpcId', params: { vpcId: related.id } })
      return
    }
    navigate({ to: '/subnets/$subnetId', params: { subnetId: related.id } })
  }
  const unavailable = (description: string) => <Empty description={description} />
  return (
    <DetailPageFrame
      breadcrumbs={[{ label: '网络' }, { label: '负载均衡', to: '/load-balancers' }, { label: item.name }]}
      title={item.name}
