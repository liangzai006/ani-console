...
type RelatedResource = {
  ...
  name: string
  status: string
  route: '/vpcs/$vpcId' | '/compute-instances/$instanceId'
}
...
export function SecurityGroupDetailPage({ securityGroupId }: { securityGroupId: string }) {
  ...
  if (!detail.data)
    return (
      <DetailPagePlaceholder
        breadcrumbs={[{ label: '网络' }, { label: '安全组', to: '/security-groups' }, { label: securityGroupId }]}
  ...
  const computeRelatedResources: RelatedResource[] = securityGroupBindings.map((binding) => {
      ...
      return {
        ...
        route: '/compute-instances/$instanceId',
      }
    })
