...
export function requireIpv4Cidr(value: string, label: string): string {
  ...
  const prefixNumber = Number(prefix)
  if (extra !== undefined || !isIpv4(ip) || !/^\d+$/.test(prefix ?? '') || prefixNumber < 0 || prefixNumber > 32) {
    throw new Error(`${label}必须是有效的 IPv4 CIDR`)
  }
  return trimmed
}

export function requireIpv4CidrWithin(value: string, parentCidr: string, label: string, parentLabel: string): string {
  const trimmed = requireIpv4Cidr(value, label)
  const child = parseIpv4Cidr(trimmed)
  const parent = parseIpv4Cidr(parentCidr)
  if (child.ip < parent.ip || child.ip + child.size > parent.ip + parent.size || child.prefix < parent.prefix) {
...
export function ipv4CidrWithinError(value: string, parentCidr: string, label: string, parentLabel: string): string | undefined {
  try {
...
export function optionalIpv4WithinCidr(value: string, cidr: string, label: string, cidrLabel: string): string | undefined {
  const trimmed = optionalIpv4(value, label)
  if (!trimmed) return undefined
  const ip = trimmed.split('.').map(Number).reduce((sum, byte) => sum * 256 + byte, 0)
