...
export const Route = createFileRoute('/_authenticated/volumes/')({ component: VolumesPage })
...
function VolumesPage() {
  ...
  const volumes = useQuery({
    queryKey: ['volumes'],
    queryFn: () => listOrThrow(() => coreApi.GET('/volumes', { params: { query: { limit: 100 } } })),
  })
  const deleteVolume = useMutation({
    mutationFn: async (item: Volume) => {
      const { error } = await coreApi.DELETE('/volumes/{volume_id}', { params: { path: { volume_id: item.id } } })
  ...
  const statusCounts = useMemo(
    () => ({
      all: items.length,
      available: items.filter((item) => item.state === 'available' && !isMounted(item)).length,
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      if (status === 'available' && !(item.state === 'available' && !isMounted(item))) return false
      if (status === 'mounted' && !isMounted(item)) return false
      if (status === 'failed' && item.state !== 'failed') return false
      return !keyword || String(item[searchField]).toLowerCase().includes(keyword)
    })
  ...
  const columns: Array<ListColumn<Volume>> = [
    ...
    { key: 'size', title: '容量 (GiB)', width: 110, render: (item) => item.size_gib },
    { key: 'storageClass', title: '类型', minWidth: 150, render: (item) => item.storage_class },
    {
      key: 'encrypted',
      title: '加密',
      width: 90,
    ...
    { key: 'zone', title: '可用区', minWidth: 120, render: (item) => item.zone ?? '—' },
    {
      key: 'mountInstance',
      title: '挂载实例',
      minWidth: 180,
    ...
    <>
        ...
        tabs={
          <StatusTabs
            ...
            items={[
              ...
              { value: 'mounted', label: '已挂载', count: statusCounts.mounted },
      ...
      >
        <DataTable
          ...
          error={volumes.error ? getErrorMessage(volumes.error, '块存储卷列表加载失败') : null}
          onRetry={() => void volumes.refetch()}
          emptyIconClassName="icon-kuaicunchu"
          emptyText={searchText || status !== 'all' ? '没有符合条件的块存储卷' : '还没有块存储卷，点击「创建卷」开始'}
          tableLabel="块存储卷列表"
          preserveTableOnEmpty
          renderRowActions={(item) => (
            <ListRowActions>
              <ListRowActionButton
                onClick={() => navigate({ to: '/volumes/$volumeId', params: { volumeId: item.id } })}
              >
                详情
              </ListRowActionButton>
              <ListRowActionButton
