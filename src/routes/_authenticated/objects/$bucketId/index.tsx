...
export const Route = createFileRoute('/_authenticated/objects/$bucketId/')({ component: BucketDetailPage })
...
function BucketDetailPage() {
    ...
    queryFn: () =>
      listOrThrow(() =>
        coreApi.GET('/buckets/{bucket_id}/objects', {
          params: { path: { bucket_id: bucketId }, query: { prefix, limit: 100 } },
        }),
      ),
    enabled: !!bucket.data,
  })
  const lifecycleRules = useQuery({
    ...
    queryFn: () =>
      listOrThrow(() =>
        coreApi.GET('/buckets/{bucket_id}/lifecycle-rules', { params: { path: { bucket_id: bucketId } } }),
      ),
    enabled: !!bucket.data,
  })
  ...
  const updateAcl = useMutation({
    mutationFn: async (_: undefined) => {
      const { data, error } = await coreApi.PUT('/buckets/{bucket_id}/acl', {
        params: { path: { bucket_id: bucketId } },
        body: { acl: aclDraft ?? 'private', idempotency_key: newIdempotencyKey() },
      })
      if (error) throw error
      return data
    },
  ...
  const updateClass = useMutation({
    mutationFn: async (_: undefined) => {
      const { data, error } = await coreApi.PUT('/buckets/{bucket_id}/storage-class', {
        params: { path: { bucket_id: bucketId } },
        body: { storage_class: classDraft ?? 'standard', idempotency_key: newIdempotencyKey() },
      })
      if (error) throw error
      return data
    },
  ...
  if (bucket.error || !bucket.data)
    return <ApiErrorAlert error={bucket.error ?? new Error('存储桶不存在或无权访问')} title="存储桶加载失败" />
  ...
  return (
    <>
      <DetailPageFrame
        breadcrumbs={[{ label: '存储' }, { label: '对象存储', to: '/objects' }, { label: bucketInfo.name }]}
        ...
        headerItems={[
          ...
          { label: '对象数', value: bucketInfo.object_count ?? entryItems.length },
        ...
        cards={[
          {
            ...
            fields: [
              ...
              { label: '创建时间', value: formatDateTime(bucketInfo.created_at) },
              { label: '更新时间', value: formatDateTime(bucketInfo.updated_at) },
            ],
          },
        ]}
        tabs={[
          {
            ...
            extra: (
              <Space>
                ...
                <Button onClick={() => setObjectVisible(true)}>登记元数据</Button>
              </Space>
            ),
            content: (
              bucketEntries.error ? (
          ...
          },
          {
            ...
            content: (
              <Space direction="vertical" size={20} className="w-full">
                ...
                />
                <Space>
                  <Select value={aclDraft ?? 'private'} onChange={setAclDraft} style={{ width: 180 }}>
                ...
                {lifecycleRules.error ? (
                  ...
                  <Table<LifecycleRule>
                    columns={[
                      ...
                      { title: '前缀', render: (_, row) => row.prefix || '全部' },
                      { title: '转低频天数', dataIndex: 'to_infrequent_days', width: 120 },
                      { title: '过期天数', dataIndex: 'expire_days', width: 120 },
                      {
                        title: '状态',
                        width: 100,
                        render: (_, row) => <Tag color={row.enabled ? 'green' : 'gray'}>{row.enabled ? '启用' : '停用'}</Tag>,
                      },
                      {
                        title: '操作',
                        width: 160,
          ...
          },
          {
            ...
            content: (
              <Space direction="vertical" size={20} className="w-full">
                <Typography.Text type="secondary">
                  以下信息用于 S3 兼容 SDK 与 CLI 接入。访问凭据请前往「设置 → API Key」管理。
                </Typography.Text>
                <Descriptions
                  column={1}
                  border
                  data={[
                    { label: 'Region', value: bucketInfo.region ?? '—' },
                    {
                      ...
                      value: (
                        <Space>
                          ...
                          {bucketInfo.endpoint ? (
                            <Button type="text" size="mini" onClick={() => copyText(bucketInfo.endpoint!, 'Endpoint 已复制')}>
                              复制
                            </Button>
                          ) : null}
                        </Space>
                    ...
                    { label: '版本控制', value: bucketInfo.versioning === 'enabled' ? '开启' : '关闭' },
                    { label: '存储类型', value: storageClassLabel },
                  ]}
                />
                <Space>
                  <Select value={classDraft ?? 'standard'} onChange={setClassDraft} style={{ width: 180 }}>
                  ...
                  <Button
                    ...
                    disabled={(classDraft ?? 'standard') === (bucketInfo.storage_class ?? 'standard')}
