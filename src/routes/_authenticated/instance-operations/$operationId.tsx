...
function OperationDetailPage() {
  ...
  if (isLoading && !data)
    return (
      <div className="space-y-5">
        <PageHeader title={`操作 ${operationId}`} subtitle="加载中…" />
        <div className="flex justify-center py-16">
          <Spin />
        </div>
      </div>
    );
  const stepItems = (data?.steps ?? []) as {
    step_name?: string;
    status?: string;
    message?: string;
  ...
  const operationStatus = data?.failure_reason ? (
    <Tooltip content={data.failure_reason}>
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        extra={<Link to="/instances">返回实例</Link>}
      />
      <Card>
        <Descriptions
          column={{ xs: 1, sm: 2 }}
          data={[
            { label: "操作", value: data?.operation },
            { label: "状态", value: operationStatus },
            { label: "实例", value: data?.instance_id },
            { label: "发起人", value: data?.requested_by },
      ...
      <Card title="执行步骤">
        <DataTable
          columns={[
            ...
            {
              title: "开始时间",
              render: (_, r) => formatDateTime(r.started_at),
            },
            {
              title: "结束时间",
              render: (_, r) => formatDateTime(r.completed_at),
            },
