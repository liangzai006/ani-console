...
function VpcDetailPage() {
  ...
  const associatedLoadBalancers = ((loadBalancers.data?.items ?? []) as LoadBalancer[]).filter(
  ...
  const relatedResources: RelatedResource[] = [
    ...vpcSubnets.map((item) => ({
      ...
      status: item.state,
      group: '网络' as const,
      route: '/networks/subnets' as const,
    })),
    ...associatedSecurityGroups.map((item) => ({
      ...
      status: item.state,
      group: '网络' as const,
      route: '/networks/security-groups' as const,
    })),
    ...vpcRoutes.map((item) => ({
      id: item.id,
      kind: '路由' as const,
      ...
      status: '—',
      group: '网络' as const,
      route: '/networks/routes' as const,
    })),
    ...
    ...associatedInstances.map((item) => ({
      ...
      status: item.state,
      group: '算力' as const,
      route: '/instances/$instanceId' as const,
    })),
  ]
  ...
  return (
    <DetailPageFrame
      breadcrumbs={[{ label: '网络' }, { label: 'VPC', to: '/networks/vpcs' }, { label: vpc.name }]}
      title={vpc.name}
      ...
      onBack={() => navigate({ to: '/networks/vpcs' })}
    />
  )
}
