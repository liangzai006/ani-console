    ...
    <Menu onClickMenuItem={onAction}>
      {row ? (
        <>
          ...
          {row.status === "running" ? (
            <Menu.Item key="console">远程连接</Menu.Item>
          ) : null}
        </>
      ) : null}
...
export function VmInstancesPage({
  dataSource = vmInstanceDataSource,
}: {
  dataSource?: VmInstanceDataSource;
}) {
  ...
  const powerMutation = useMutation({
    mutationFn: ({
      ...
      ids: string[];
      action: VmInstancePowerAction;
    }) => dataSource.changePowerState(ids, action),
    onSuccess: (_, variables) => {
      Message.success(
        variables.action === "start" ? "启动操作已提交" : "停止操作已提交",
      );
      setSelectedKeys([]);
  ...
  const changePowerState = (
    ...
    powerMutation.mutate({
      ids: compatibleRows.map((item) => item.id),
      action,
    });
  };
  ...
  const allColumns: Array<ListColumn<VmInstance>> = [
    ...
    {
      key: "spec",
      title: COLUMN_LABELS.spec,
      render: (_, row) => row.spec,
    },
    {
      key: "image",
    ...
    {
      key: "privateIp",
      title: COLUMN_LABELS.privateIp,
      render: (_, row) => row.privateIp,
    },
    {
      key: "node",
    ...
    },
    {
      ...
      render: (_, row) => <ProtectionCell enabled={row.terminationProtected} />,
    },
    {
      key: "createdAt",
  ...
  const statusTabs = [
    ...
    },
    {
      ...
      label: "异常",
      count: result.statusCounts.error,
    },
  ];
          ...
          actions={
            <>
              ...
              <Dropdown
                ...
                droplist={
                  <ActionMenu onAction={(action) => notifyMenuAction(action)} />
                }
              >
                <ToolbarButton
              ...
              <DataTableRowActions>
                ...
                <Dropdown
                  ...
                  droplist={
                    <ActionMenu
                      row={row}
                      onAction={(action) => notifyMenuAction(action, row)}
                    />
                  }
                >
                  <DataTableRowActionButton>
